...
class SSEConsumerTest(TestCase):
    ...
    async def test_new_consumer_instance(self):
        ...
        final_route = urljoin(
            self.consumer_params[0], self.one_route_fixture["routes"][0]
        )
    ...
    def test_new_consumer_instance_multiple_routes(self):
        ...
        expected_routes = [
            ...
            urljoin(
                self.consumer_params[0], self.one_route_fixture["routes"][1]
            ),
        ]
    ...
    async def test_follow_redirect(self):
        ...
        with asynctest.patch.object(
            ...
            with aioresponses() as m:
                m.get(
                    "http://localhost:8081/v2/events",
                    status=301,
                    headers={"Location": "http://localhost:8080/v2/events"},
                )
                m.get(
                    "http://localhost:8080/v2/events", status=200, body=content
                )
    ...
    async def test_call_on_event_when_sse_event_is_found(self):
        ...
        with asynctest.patch.object(
            self.consumer, "keep_runnig", side_effect=[True, False]
    ...
    async def test_reconnect_if_disconnected(self):
        ...
        with asynctest.patch.object(
            self.consumer, "keep_runnig", side_effect=[True, True, True, False]
        ), asynctest.patch.object(
            self.consumer, "_consume_events", side_effect=CoroutineMock()
        ) as consume_events_mock:
            await self.consumer.start()
    ...
    async def test_consume_again_if_reconnected(self):
        session_mock = CoroutineMock(
            get=CoroutineMock(side_effect=["", aiohttp.ClientError(), ""])
        )
        self.consumer.session = session_mock
    ...
    async def test_call_on_connection_error(self):
        ...
        with asynctest.patch.object(
            self.consumer, "keep_runnig", side_effect=self.total_loops(1)
        ), asynctest.patch.object(
            self.consumer, "on_connection_error", side_effect=CoroutineMock()
        ) as on_connection_error_mock:
            await self.consumer.start()
    ...
    async def test_call_on_exception(self):
        ...
        with asynctest.patch.object(
            ...
            self.consumer, "on_event", side_effect=Exception()
        ), asynctest.patch.object(
            self.consumer, "on_exception", side_effect=CoroutineMock()
    ...
    async def test_reconect_if_unhandled_reconnected(self):
        session_mock = CoroutineMock(
            get=CoroutineMock(side_effect=["", Exception()])
        )
        self.consumer.session = session_mock

        with asynctest.patch.object(
            self.consumer, "keep_runnig", side_effect=[True, True, False]
        ), asynctest.patch.object(
            self.consumer, "_consume_events", side_effect=CoroutineMock()
        ) as consume_events_mock:
            await self.consumer.start()
    ...
    async def test_call_on_connect_callback(self):
        ...
        with asynctest.patch.object(
            ...
            self.consumer, "on_connection", side_effect=CoroutineMock()
        ) as on_connection_mock:
            await self.consumer.start()
    ...
    async def test_call_on_connect_callback_on_reconnect(self):
        ...
        with asynctest.patch.object(
            self.consumer, "keep_runnig", side_effect=self.total_loops(3)
        ), asynctest.patch.object(
            self.consumer, "on_connection", side_effect=CoroutineMock()
        ) as on_connection_mock:
            await self.consumer.start()
