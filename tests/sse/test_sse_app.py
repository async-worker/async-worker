...
class AppTest(asynctest.TestCase):
    ...
    async def test_register_bulk_flush_timeout(self):
        ...
        routes = app.routes_registry.sse_routes
        self.assertEqual(len(routes), 1)
        self.assertEqual(expected_bulk_flush_interval, routes[0]['options']['bulk_flush_interval'])
        self.assertEqual(42, await routes[0]['handler'](None))
    ...
    async def test_instantiate_one_consumer_per_handler_one_handler_registered(self):
        ...
        self.assertEqual(self.connection_parameters['password'], consumer.password)
