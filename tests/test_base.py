...
class BaseAppTests(asynctest.TestCase):
    ...
    async def test_run_every_registers_a_coroutine_to_be_executed_as_a_ScheduledTaskRunner(self):
        with patch('asyncworker.base.ScheduledTaskRunner', spec=ScheduledTaskRunner) as Runner:
            seconds = 10
    ...
    async def test_run_every_max_concurrency_can_be_overwritten_with_options(self):
        with patch('asyncworker.base.ScheduledTaskRunner', spec=ScheduledTaskRunner) as Runner:
            ...
            self.app.run_every(seconds=seconds,
                               options={Options.MAX_CONCURRENCY: 666})(coro)
