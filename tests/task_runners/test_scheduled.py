...
class ScheduledTaskRunnerTests(asynctest.TestCase):
    ...
    async def test_it_can_dispatch_a_new_task_if_running_tasks_doesnt_exceed_max_concurrency(self):
        self.task_runner.running_tasks = {CoroutineMock()}

        self.assertTrue(await self.task_runner.can_dispatch_task())

    async def test_it_waits_for_a_task_to_be_completed_before_dispatching_a_new_task_if_running_tasks_reached_max_concurrency(self):
    ...
    async def test_wrapped_task_emits_the_task_is_done_event_if_task_finishes_successfully(self):
    ...
    async def test_wrapped_task_emits_the_task_is_done_event_if_task_raises_an_error(self):
        with patch.object(self.task_runner.task_is_done_event, 'set') as set:
    ...
    async def test_start_creates_a_background_task_for_run(self):
        ...
        with patch.object(self.task_runner, '_run') as _run, \
            patch('asyncworker.task_runners.asyncio.ensure_future') as ensure_future:
    ...
    async def test_run_follows_the_clock_tick(self):
        ...
        with patch.multiple(self.task_runner,
                            clock=clock,
    ...
    async def test_run_emits_a_task_i_done_event_for_each_valid_clock_tick(self):
        clock = asynctest.MagicMock()
        clock.__aiter__.return_value = range(3)
        wrapped_task = Mock()
        with patch.multiple(self.task_runner,
